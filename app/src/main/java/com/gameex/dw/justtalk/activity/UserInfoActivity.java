...
import static com.gameex.dw.justtalk.fragment.MyInfoFragment.UPDATE_USER_INFO;
...
public class UserInfoActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.mine_icon_info:   //修改头像
                showTypeDialog();
                break;
            case R.id.mine_icon_info_layout:    //修改昵称
                ...
                break;
            case R.id.mine_qr_code_layout:  //展示二维码
                if (mUserInfo == null) {
                    return;
                }
                mUserInfo.getAvatarBitmap(new GetAvatarBitmapCallback() {
                    ...
                    public void gotResult(int i, String s, Bitmap bitmap) {
                        LogUtil.d(TAG, "OnClick-mine_qr_code_layout: " +
                                "responseCode = " + i + " desc = " + s);
                        if (i == 0)
                            DialogUtil.showQrDialog(UserInfoActivity.this, mUserInfo.getUserName()
                                    , bitmap);
                        else DialogUtil.showQrDialog(UserInfoActivity.this, mUserInfo.getUserName()
                                , null);
                    }
                });
                break;
            case R.id.fly_code_info_layout: //飞聊号
                if (mEditPup != null) {
                    mEditPup.showAtLocation(mLinearLayout, Gravity.CENTER, 0, 0);
                    WindowUtil.showBackgroundAnimator(sUserInfoActivity, 0.5f);
                }
                break;
            case R.id.fly_sign_info_layout: //修改个性签名
                intent.setClass(this, EditMyInfoActivity.class);
                intent.putExtra("title", "个性签名");
                intent.putExtra("my_signature", mFlySignText.getText());
                startActivityForResult(intent, EDIT_SIGNATURE_REQUEST_CODE);
                break;
            case R.id.mine_num_info_layout: //修改手机号
            ...
            case R.id.take_photo:   //拍照
                requestPermission();
                break;
            case R.id.choose_from_lib_text: //从相册中选取
